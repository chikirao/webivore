...
const wanted = new Set(
  process.argv.slice(2).length ? process.argv.slice(2) : ["all"],
);
...
const checks = [],
  errors = [];
async function check(page, state, name) {
    ...
    return {
      ...
      controls: controls.map((e) => {
        ...
        return {
          ...
          width: r.width,
          height: r.height,
          unobstructed: !!hit && (hit === e || e.contains(hit)),
        };
      }),
    };
  });
  assert(
    ...
    `${state}/${name}: horizontal overflow`,
  );
  for (const c of data.controls) {
    assert(
      c.x >= -1 &&
        c.x + c.width <= data.width + 1 &&
        c.y >= -1 &&
        c.y + c.height <= data.height + 1,
      `${state}/${name}: clipped ${c.name} ${JSON.stringify(c)}`,
    );
    assert(
      c.unobstructed,
      `${state}/${name}: another element covers ${c.name}`,
    );
    if (data.width <= 800)
      assert(
        c.width >= 43.9 && c.height >= 43.9,
        `${state}/${name}: small touch target ${c.name}`,
      );
  }
  checks.push({ state, name, ...data });
}
async function collect(page, target) {
  return page.evaluate(async (target) => {
    ...
    return {
      ...
      percent: (100 * game.mass) / game.total,
      radius: game.radius,
    };
  }, target);
}
try {
  for (const [name, width, height] of viewports) {
    ...
    if (want("entry")) {
      await check(page, "entry", name);
      await page.screenshot({ path: `artifacts/shots/entry-${name}.png` });
    }
    if (!(want("game") || want("grown") || want("finish"))) {
      await page.close();
      continue;
    }
    ...
    await wait(3800);
    if (want("game")) {
      await check(page, "game", name);
      await page.screenshot({ path: `artifacts/shots/game-start-${name}.png` });
    }
      ...
      for (const [label, pitch] of [
        ...
        if (want("grown"))
          await page.screenshot({
            path: `artifacts/shots/game-68-${label}-${name}.png`,
          });
      }
    }
    if (want("finish")) {
  ...
  await browser.close();
  writeFileSync(
    "artifacts/astra-qa/responsive-results.json",
